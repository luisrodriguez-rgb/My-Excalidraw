...
Community libraries (https://libraries.excalidraw.com, repo excalidraw/excalidraw-libraries) are almost all built from the same vector primitives this skill already exports (rectangle/ellipse/line/arrow/diamond/text/freedraw), so their items render correctly through Kroki and the local brute-export CLI.

This helper discovers libraries, lists their items, and merges an item into a scene with namespaced IDs and translated coordinates so it drops in without collisions.
...
<source> is "author/name.excalidrawlib" (fetched + cached from the official repo) or a path to a local .excalidrawlib file.
"""
...
def _elem_bounds(e):
    """True visual bounds of one element — accounts for `points` (lines/arrows) and `angle` (rotation), both of which a naive x/y/w/h box misses and which otherwise mis-place rotated or arrow-based library icons."""
