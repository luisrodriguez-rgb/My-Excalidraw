...
On-demand depth for the `excalidraw` skill. Read this when you need a copy-paste starting point or the full catalogue of a property's values. The core generate-time rules (binding, endpoints, sizing, arrow labels) live in `SKILL.md` — this file is the long tail, not a replacement.

## Copy-paste element templates
All templates use descriptive string IDs and section-namespaced seeds (100xxx). Swap colors from the palette in `SKILL.md`.
...
Arrows default to `endArrowhead: "arrow"`, `startArrowhead: null` — omit both for a standard one-way arrow.
